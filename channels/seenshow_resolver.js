...
const _rawProxy = process.env.SEENSHOW_AUTH_PROXY;
const AUTH_PROXY = (_rawProxy === 'none' || _rawProxy === '') ? '' : (_rawProxy || 'socks5://127.0.0.1:9050');
const CURL_INTERFACE = process.env.SEENSHOW_CURL_INTERFACE || 'tun1'; // network interface for curl requests
const UPSTREAM_BASE = 'https://live.seenshow.com'; // origin whose paths are proxied through /proxy/hls/
...
async function ensureBrowser() {
  ...
  browserLaunchPromise = (async () => {
    ...
    // Force DNS resolution for Akamai CDN to UAE edge (bypasses snap DNS isolation)
    const HOST_RESOLVER_RULES = process.env.SEENSHOW_HOST_RESOLVER_RULES || '';
    if (HOST_RESOLVER_RULES) {
      args.push(`--host-resolver-rules=${HOST_RESOLVER_RULES}`);
      log(`Using host resolver rules: ${HOST_RESOLVER_RULES}`);
    }
...
async function promoteHdntsToHdntl(page, channelId, manifestUrl) {
  ...
  // Use curl (not Node.js HTTP) for the hdnts→hdntl promotion.
  // Akamai uses TLS fingerprinting (JA3/JA4) and the hdntl HMAC is bound to
  // properties of the TLS session. Node.js and curl have different TLS fingerprints,
  // so tokens generated by Node.js won't validate when consumed by curl/FFmpeg.
  // Using curl here ensures the hdntl tokens are compatible with FFmpeg's TLS stack.
  //
  // IMPORTANT: do NOT fall back to httpGetText on curl failure. Node HTTP will
  // produce a token bound to Node's TLS fingerprint which FFmpeg cannot
  // validate — the stream would 403 downstream with no obvious cause. On curl
  // failure, return null and let the caller rebuild via the browser path.
  if (!isTun1Up()) {
    warn(`Promotion aborted for ${channelId}: tun1 interface is not UP (seenshow requires --interface tun1)`);
    return null;
  }
  let payload;
  try {
    const result = await curlGetText(manifestUrl, 15000);
    payload = {
      status: result.status,
      finalUrl: result.finalUrl || manifestUrl,
      text: result.body || '',
    };
    log(`Promotion via curl: ${result.status} for ${channelId} (${(result.body || '').length} bytes)`);
  } catch (err) {
    warn(`Promotion curl failed for ${channelId}: ${err.message} — NOT falling back to Node HTTP (TLS fingerprint mismatch would produce invalid token)`);
    return null;
  }
  ...
  if (bestVariant && bestVariant.includes('hdntl=')) {
    ...
    if (process.env.SEENSHOW_DEBUG_PROMOTION) {
      log(`  promotion debug: status=${payload.status}, finalUrl=${(payload.finalUrl || '').slice(0, 500)}`);
      log(`  promotion debug: bestVariant=${bestVariant.slice(0, 300)}`);
      log(`  promotion debug: playlist snippet=${(payload.text || '').slice(0, 500).replace(/\n/g, '\\n')}`);
    }
    return {
      url: bestVariant,
...
async function extractToken(channelId, channelConfig) {
  ...
  try {
    ...
    const directCandidates = [
      `${UPSTREAM_BASE}/hls/live/${channelConfig.hls_path}/master.m3u8`,
      `${UPSTREAM_BASE}/hls/live/${channelConfig.hls_path}/3.m3u8`,
    ];
        ...
        try {
          ...
          if (entry.body.includes(channelConfig.hls_path)) {
            ...
            if (hdntlMatch) {
              ...
              const inferredUrl = `${UPSTREAM_BASE}/hls/live/${channelConfig.hls_path}/master.m3u8?hdntl=${hdntlRaw}`;
...
/**
 * Build the argv for curl. All call sites go through this so the shared flags
 * (tunnel interface, silent-but-show-errors, timeout) stay in one place.
 * mode:
 *   'status'      — capture body to stdout + print http_code marker (curlGetText)
 *   'dumpHeaders' — dump upstream headers then body to stdout (/proxy GET)
 *   'head'        — curl --head, no body (/proxy HEAD)
 */
function buildCurlArgs({ url, timeoutSec = 15, mode }) {
  const args = [
    '-sS',                          // silent but show errors
    '--interface', CURL_INTERFACE,   // force through VPN tunnel
    '--max-time', String(timeoutSec),
  ];
  if (mode === 'status') {
    args.push('-o', '-', '-w', '\n__CURL_STATUS__:%{http_code}');
  } else if (mode === 'dumpHeaders') {
    args.push('-D', '-');
  } else if (mode === 'head') {
    args.push('--head');
  }
...
/**
 * Fetch a URL using curl subprocess. This is critical for Akamai EdgeAuth:
 * the hdntl HMAC is bound to TLS fingerprint properties, so the same TLS
 * client (curl/libcurl) must be used for both promotion and consumption.
 * FFmpeg uses libcurl internally, so curl-generated tokens are compatible.
 */

// isTun1Up — best-effort check that the VPN interface curl binds to is
// actually up. Returns false if /sys/class/net/<iface>/operstate is not "up".
// Non-authoritative (interface can go down between the check and the curl
// spawn) but catches the common case where OpenVPN is down and every curl
// call would fail wasting timeout budget.
function isTun1Up() {
  const iface = CURL_INTERFACE;
  if (!iface) return true;
  try {
    const state = require('fs').readFileSync(`/sys/class/net/${iface}/operstate`, 'utf8').trim();
    return state === 'up' || state === 'unknown'; // tun/tap often reports "unknown"
  } catch {
    return false;
  }
}

function curlGetText(url, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    const timeoutSec = Math.max(1, Math.ceil(timeoutMs / 1000));
    const args = buildCurlArgs({ url, timeoutSec, mode: 'status' });
    const child = require('child_process').spawn('curl', args, {
    ...
    child.on('close', (code) => {
      ...
      if (code !== 0 && httpStatus === 0) {
        reject(new Error(`curl exit ${code}: ${stderr.trim()}`));
        return;
      }
      resolve({ status: httpStatus, body, finalUrl: url });
    });
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /proxy/hls/... — Relay HLS content via curl (TLS fingerprint compatible)
    // FFmpeg can fetch from http://localhost:8090/proxy/hls/live/...
    // instead of https://live.seenshow.com/hls/live/...
    // Using curl ensures the TLS fingerprint matches the one used during token
    // promotion, so hdntl tokens validate correctly.
    if ((method === 'GET' || method === 'HEAD') && pathname.startsWith('/proxy/')) {
      const isHead = method === 'HEAD';
      const remotePath = pathname.slice('/proxy'.length);

      // Security: restrict proxied paths to /hls/ to prevent probing arbitrary
      // paths on the upstream CDN (SSRF mitigation).
      if (!remotePath.startsWith('/hls/')) {
        sendJson(res, 400, { error: 'Only /hls/ paths are proxied' });
        return;
      }

      const fullParsed = new URL(req.url, 'http://localhost');
      const remoteUrl = `${UPSTREAM_BASE}${remotePath}${fullParsed.search || ''}`;

      // For HEAD requests, use curl --head to avoid downloading full content
      try {
        const curlArgs = buildCurlArgs({
          url: remoteUrl,
          timeoutSec: 15,
          mode: isHead ? 'head' : 'dumpHeaders',
        });
        ...
        // Once headers are parsed, body streaming is handled by dedicated
        // inner handlers (playlist buffering or binary pipe). The outer handler
        // only processes the initial header accumulation phase.
        let bodyHandlerInstalled = false;

        child.stdout.on('data', (chunk) => {
          // After headers are parsed and body handler is installed, skip
          if (bodyHandlerInstalled) return;
          ...
          // Parse headers
          const headerStr = headerBuf.slice(0, headerEnd);
          const bodyStart = headerBuf.slice(headerEnd + 4);
          headersParsed = true;

          const statusLine = headerStr.split('\r\n')[0] || '';
          const statusMatch = statusLine.match(/HTTP\/[\d.]+ (\d+)/);
          if (statusMatch) upstreamStatus = parseInt(statusMatch[1], 10);
          ...
          if (upstreamStatus >= 400) {
            ...
            res.end(isHead ? undefined : `Upstream ${upstreamStatus}`);
            child.kill();
            return;
          }

          // HEAD: just return status + headers, no body
          if (isHead) {
            res.writeHead(upstreamStatus, { 'Content-Type': contentType });
            res.end();
            child.kill();
            return;
          }
          ...
          if (isPlaylist) {
            // Buffer text playlists to rewrite URLs
            let playlistBuf = bodyStart;
            child.stdout.on('data', (c) => { playlistBuf += c.toString('binary'); });
            child.on('close', () => {
              // Rewrite absolute seenshow URLs to go through proxy
              // Use configured HOST:PORT instead of req.headers.host to prevent
              // Host header injection from poisoning rewritten playlist URLs.
              const proxyBase = `http://${HOST}:${PORT}`;
              let text = playlistBuf.replace(/https:\/\/live\.seenshow\.com\//g, `${proxyBase}/proxy/`);
              res.end(text);
            });
          } else {
            // Stream binary data (TS segments) directly. Use pipe() so Node
            // handles backpressure automatically (pauses curl's stdout when
            // the client socket buffers fill up). The previous manual
            // `res.write(c)` ignored the false-return signal and could
            // grow memory unboundedly when a downstream HLS client over a
            // geo-routed CDN was slow to drain.
        ...
        child.stderr.on('data', () => {}); // ignore curl stderr
        // If curl closes before headers were parsed (e.g. tun1 down, DNS/TLS failure),
        // return a bounded error response instead of hanging the client forever.
        // For HEAD: attempt partial header parse. For GET: return 502 immediately.
        child.on('close', (code) => {
          if (!headersParsed && !res.headersSent) {
            if (isHead) {
              const statusMatch = headerBuf.match(/HTTP\/[\d.]+ (\d+)/);
              const status = statusMatch ? parseInt(statusMatch[1], 10) : 502;
              res.writeHead(status, { 'Content-Type': 'application/octet-stream' });
              res.end();
            } else {
              res.writeHead(502, { 'Content-Type': 'text/plain' });
              res.end(`Upstream unavailable (curl exit ${code})`);
            }
          }
        });
        child.on('error', (err) => {
          if (!res.headersSent) {
            sendJson(res, 502, { error: `curl error: ${err.message}` });
          }
        });
      } catch (err) {
        error(`Proxy error for ${remotePath}: ${err.message}`);
        if (!res.headersSent) {
